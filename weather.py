...
def getVilageFcst(date, time, nx, ny):
  ...
  result = dict(map(lambda item: (item['category'], {'value': item['fcstValue'], 'date': item['fcstDate'], 'time': item['fcstTime']}), result['body']['items']['item']))
  print(result)
  ...
  print('풍속:', result.get('VVV', NULL_DATA).get('value'), result.get('UUU', NULL_DATA).get('value'))
